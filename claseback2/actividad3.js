/*Ejercicio 3: Módulos y require  
1.Dentro de la carpeta clase2, crea un archivo llamado datos.js que 
contenga un objeto con información sobre un curso, similar a los ejemplos 
anteriores.  
2.Crea otro archivo llamado actividad3.js que importará los datos de 
datos.js usando require(). 
Instrucciones: 
• En datos.js, define un objeto que tenga información como 
nombreCurso, duracion, y temas (array de cadenas). 
• Exporta este objeto para que pueda ser utilizado en otros archivos. 
• En actividad3.js, importa el objeto usando require(). 
• Muestra en la consola la información del curso.*/


// Importamos el objeto desde datos.js
const curso = require("./datos");

// Mostramos la información del curso en la consola
console.log("Información del curso:");
console.log("Nombre:", curso.nombreCurso);
console.log("Duración:", curso.duracion);
console.log("Temas:", curso.temas);

// Exportamos el curso para que pueda ser utilizado en otros archivos
module.exports = curso;
// Ejemplo de uso
console.log("Curso importado correctamente:", curso);
// Exportamos la función para que pueda ser utilizada en otros archivos
// Ejemplo de uso
// console.log("Curso importado correctamente:", curso);


/*Información del curso:
Nombre: Introducción a Node.js
Duración: 4 semanas
Temas: [ 'Módulos', 'NPM', 'JSON', 'File System' ]
Curso importado correctamente: {
  nombreCurso: 'Introducción a Node.js',
  duracion: '4 semanas',
  temas: [ 'Módulos', 'NPM', 'JSON', 'File System' ]
}*/
